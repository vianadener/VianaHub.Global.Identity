# Design Técnico - Resource Production Validation

## Objetivo

Validar tecnicamente a implementação do recurso `Resource` para confirmar que endpoints, serviço de aplicação, domínio, persistência e testes estão coerentes com a arquitetura do projeto.

## Escopo técnico

- API Minimal para `Resource`
- Serviço de aplicação `ResourceAppService`
- Serviço de domínio `ResourceDomainService`
- Entidade `ResourceEntity`
- Repositório `ResourceDataRepository`
...
- `src/VianaHub.Global.Identity.Infra.Data/Repository/ResourceDataRepository.cs`
- `src/VianaHub.Global.Identity.Infra.Data/Mappings/ResourceMapping.cs`
...
1. Listagem paginada de resources
2. Consulta por id
3. Criação
4. Atualização
...
- Os testes cobrindo `Resource` passam.
